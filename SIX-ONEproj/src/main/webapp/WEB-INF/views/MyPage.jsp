...
function ChattingList(el){
	...
	var appendmMessage = function(msg){
		//메시지 출력
		MsgSplit = msg.split(':');   
		console.log(msg);  ///이길동:ㄹ홓로:김길동
		if(MsgSplit[0] === Opponentnickname && MsgSplit[2] === nickname) {//상대가 보냈을때 메시지 출력창
	...
	var receiveMessage = function(e){
		...
		if(receiveData.substring(0,4) == 'msg:') {///상대방 메시지 뿌려주는곳
			...
			//div에 출력
			//console.log('receiveData:',receiveData.substring(4)+':'+nickname); //msg:이길동:111:'김길동'  ,   msg:이길동:111:'박길동'
			appendmMessage(receiveData.substring(4)+':'+nickname);
			 $('#chatMessage')
	            .stop()
	            .animate({ scrollTop: $('#chatMessage')[0].scrollHeight }, 300);
		}
	...
	function sendMessage(){////나의메시지 뿌려주는곳
		...
		appendmMessage(nickname+':'+$('#message').val()+':'+Opponentnickname);
		//기존 메시지 지우기
		$('#message').val('');
		//포커스 주기
		$('#message').focus();
		...
		<div class="tab-pane fade" id="qwe">
			<div class="animate-box" style="margin: 30px auto auto auto; width: 100%;">
				<div class="panel panel-danger">
					...
					<c:forEach items="${list}" var="item">
							<span style="color: black; width: 280px;" class="glyphicon glyphicon-scale"> ${item.name}님의현재몸무게-<span id ="weight" >${item.weight}</span>kg</span>
					...
					<div class="panel-footer">
					<c:forEach items="${list}" var="item">
						<span style="color: black;" class="glyphicon glyphicon-link" id="calor"> ${item.name}님의체지방률-</span>
						</c:forEach>
						<div class="progress" style="margin-top: 10px;">
					...
					<div class="panel-footer">
					<c:forEach items="${list}" var="item" varStatus="loop">
						<span style="color: black;" class="glyphicon glyphicon-piggy-bank" id="Beman"> ${item.name}님의비만도-</span>
						</c:forEach>
						<div class="progress" style="margin-top: 10px;">
					...
					<div class="panel-footer">
					<c:forEach items="${list}" var="item" varStatus="loop">
						<span style="color: black;" class="glyphicon glyphicon-apple" id="daesalyang"> ${item.name}님의기초대사량-</span>
					</c:forEach>
		...
		<div class="tab-pane fade" id="asd">
			...
			<div class="animate-box"style="margin: 30px auto auto auto; width: 100%;">
				<div class="panel panel-danger">
					<div class="panel-body">
					<c:forEach items="${list}" var="item" varStatus="loop">
						<span style="color: black;" class="glyphicon glyphicon-cutlery"> ${item.name}님의식단리스트</span>
						</c:forEach>
					</div>
					<div class="panel-footer" >
							<c:if test="${not empty food }" var="flag">
							<c:forEach items="${food}" var="item" begin="1" end="1">
							<fmt:parseDate var="data" value="${item.foodDate}" pattern="yyyy-MM-dd"/>
							<fmt:formatDate value="${data}" pattern="yyyy-MM-dd" var="data"/>
							<h3 style="text-align: center; display: inline-block;" id="h3"><c:out value="${data}"/> 식단표</h3>
							<a href="<c:url value='/foodreset.do'/>" class="myButton">초기화</a>
							  ...
							  <div>
							    ...
							    <li style="font-weight: bold;">●&nbsp;&nbsp;${item.foodName} - ${item.foodKcal}kcal</li>
							    </c:if>
							    </c:forEach>
							    <fmt:parseNumber var="bkcal" integerOnly="true" value="${bkcal}" />
							    <li style="font-weight: bold;">●&nbsp;&nbsp;아침식단 총 칼로리 - <c:out value="${bkcal}"/>kcal</li>
							    </ul>
							  </div>
							  <h3>점심식단</h3>
							  <div>
							    ...
							    <fmt:parseNumber var="lkcal" integerOnly="true" value="${lkcal}" />
							     <li style="font-weight: bold;">●&nbsp;&nbsp;점심식단 총 칼로리 - <c:out value="${lkcal}"/>kcal</li>
							    </ul>
							   </div>
							  <h3>저녁식단</h3>
							  <div>
							    ...
							    <fmt:parseNumber var="dkcal" integerOnly="true" value="${dkcal}" />
							    <li style="font-weight: bold;">●&nbsp;&nbsp;저녁식단 총 칼로리 - <c:out value="${dkcal}"/>kcal</li>
							    </ul>
							  </div>
							  <h3>총 식단 칼로리</h3>
							  <div>
							    ...
							    <fmt:parseNumber var="kcal" integerOnly="true" value="${kcal}" />
							    <li style="font-weight: bold;">●&nbsp;&nbsp;총 칼로리 - <c:out value="${kcal}"/>kcal</li>
							    </ul>
							  </div>
							</div>
							</c:if>
						...
						<c:if test="${not flag}">
						<h3 style="text-align: center; padding-top: 25px; color: #DCDCDC">아직 식단이 없습니다</h3>
